...
- Login to the IBM Cloud cluster where your sandbox-team-one ocp cluster resides.
...
- Navigate to your namespace where you are running the inventory solution pipeline and create the binding secret for He the Appid instance on the cloud account
  ```bash 
     ...
     ibmcloud oc cluster service bind --cluster sandbox-team-one 
     --namespace [YOUR-NAMESPACE] --service sandbox-team-one-appid
  ```
...
- Add the following parameters in `values.yaml` along with its values:
   ...
   ```bash
    appidBinding: "binding-sandbox-team-one-appid"
