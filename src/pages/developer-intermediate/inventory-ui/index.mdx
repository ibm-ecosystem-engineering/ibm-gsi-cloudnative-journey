...
- Create Component View For access the StockItemList `UI-pattern`.


```javascript path=client/src/components/list/StockItemList.jsx
...
- Make a copy of the `TableList` pattern  from the `ui-patterns/list` directory. Rename the file and the class inside to `StockItemList`.
...
- Create the Model class for StockItem in the `models/list` directory.

```javascript path=client/src/models/list/StockItem.ts
...
-  Navigate to `services/list` folder under the `src` folder

- Create a file named `stock-item-mock.service.js and stock-item-mock.api.ts` in the service directory. Our
StockItem service component will have a single asynchronous function called `getStockItems()`
that returns a list of StockItems.

```javascript path=src/services/list/stock-item-mock.api.ts
...
```javascript path=src/services/list/stock-item-mock.service.js
...
```
- Update Mocke service into the  'StockItemList.tsx' component view in `components/list/StockItemList` directory.

```javascript path=src/components/list/StockItemList.tsx
import React, { Component } from "react";
import StockItemList from '../../ui-patterns/list/StockItemList';
import { Container } from 'typescript-ioc';
...
- Update `StockItemList` in `ui-patterns` to use the provided service
...
- Create the API implementation in the `services/list` directory called `stock-item.api.js`

```javascript path=src/services/list/stock-item.api.ts
import { StockItem } from "../../models/list/StockItem";
export abstract class StockItemApi {
    abstract async getStockItems(): Promise<StockItem[]>;
}
...
- Create a service implementation in the `services/list` directory called `stock-item.service.js`

```javascript path=src/services/list/stock-item.service.js
...
- Add an implementation of `getStockItems()` that calls the BFF through the `/api` proxy
...
- Update `StockItemList.tsx`  in the `src/components/list/` directory to use the new service instead of the mock service.
