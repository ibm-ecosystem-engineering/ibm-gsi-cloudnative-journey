...
This Micro App guidance continues to build upon the microserivces that were built in the Inventory Micro App guide. Make sure you have complete [Inventory App](/practical/inventory-app) or deployed the working [Inventory Solution](/practical/inventory-app#deploy-the-inventory-app-solution).
...
-  The url of the Artifactory helm repository can be determines by following these steps:
        -Log into Artifactory (use `igc credentials `to obtain your credentials and login to artifactory console).Go to the Artifactory tab and select "Artifacts".
           ![Artifactory Menu](/images/ArtifactoryMenu.png)
        - Click on Set Me Up.
        ![Set Me Up](/images/Artifactory-SetMeUp.png)
        - In the Set Me Up Section, select the tool as "Generic" and repository as "generic-local".Copy the deploy URL from the Set Me Up dialog box. That is the Artifactory helm repository URL.
        ![Artifactory Helm Repository URL ](/images/HelmRepoURL.png)
