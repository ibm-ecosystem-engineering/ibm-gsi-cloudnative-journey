...
### If you are going to use your desktop/laptop for hands on exercises, follow the instructions in Desktop/Laptop tab
### If you cannot install tools on your desktop/laptop, follow the cloud instructions
...
IBM Cloud Shell is a free service from IBM which gives you complete control of your cloud resources, applications and infrastructure, from any web browser. Get pre-authenticated access to the latest tools and programming languages for cloud-based development, deployment and management of services and applications — all in a secure shell.

Featues of IBM Cloud Shell include:
...
- Install igc by running the following commands on the just newly opened terminal:
    ...
    ```bash
    abalasu1@cloudshell:~/inventory-svc-ab$ git clone https://github.com/ibm-garage-cloud/cloud-shell-commands.git
    ...
    abalasu1@cloudshell:~/inventory-svc-ab$ export PATH=~/cloud-shell-commands:$PATH
    abalasu1@cloudshell:~/inventory-svc-ab$ source ./cloud-shell-commands/install-igc
    ...
    abalasu1@cloudshell:~/inventory-svc-ab$ 
    ```
...
- Use ibmcloud command line to log into the cloud account. Replace the user_id, password and team name in the sandbox name
```bash
ibmcloud login -u <user_id> -p <password> -g sandbox-team-n.. -r us-south
```
- Select Account Number that you are boarded on.
```bash
abalasu1@cloudshell:~/inventory-svc-ab$ ibmcloud login -u abalasu1@in.ibm.com -p XXX -g sandbox-team-one -r us-south
API endpoint: https://cloud.ibm.com
Authenticating...
OK

Select an account:
1. Arun Balasubramanyan (d7627906acd716cc06d005b524bfa02f)
2. GSI Labs - XXX (0f42478cb66d4ce0b11c9168b02c5690) <-> 2050804
3. GSI Labs - IBM (7e341a8be8c9464e8778c7107b2bcccc) <-> 1924691
Enter a number> 3
Targeted account GSI Labs - XXX (0f42478cb66d4ce0b11c9168b02c5690) <-> 2050804
...
API endpoint:      https://cloud.ibm.com   
Region:            us-south   
User:              abalasu1@in.ibm.com   
Account:           GSI Labs - XXX (0f42478cb66d4ce0b11c9168b02c5690) <-> 2050804   
...
![Get Openshift Login](./images/copylogincommand.png)

- Copy the login command and execute it from the terminal
![Openshift Login](./images/getlogincommand.png)

- Execute the login command copied in the last step in the terminal

```bash
abalasu1@cloudshell:~/inventory-svc-ab$ oc login --token=qvARHflZDlOYfjJZRJUEs53Yfy4F8aa6_L3ezoagQFM --server=https://c103-e.us-south.containers.cloud.ibm.com:30979
Logged into "https://c103-e.us-south.containers.cloud.ibm.com:30979" as "IAM#abalasu1@in.ibm.com" using the token provided.
