...
1. Make sure you have installed all the [Prerequisite Dev Tools](/getting-started/devenvsetup) before continuing
2. Log in to the development cluster's CLI (Kubernetes or Red Hat OpenShift)
...
3. You can test that you're connected to the cluster with the following simple command:
...
3. Copy the clone _SSH link_, and use the `git clone` command to clone it to your developer desktop machine.
    ```shell script
    git clone github.com/bwoolf1/stockbffnode.git
    ```
...
<Tabs>
<Tab label="OpenShift 4.3" open="true">
...
1. Run the command `igc dashboard` in your terminal to open your Developer Dashboard.
...
Having reached this point, we recommend you repeat the process a few more
 times using different **Code Pattern** templates and explore the environment
