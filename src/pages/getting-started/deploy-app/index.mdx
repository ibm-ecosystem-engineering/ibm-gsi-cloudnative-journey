---
title: Deploying an App/Create a sample application and deploy it with the CI pipeline
---
...
-  Make sure you have followed all [Prerequisites](/getting-started/prereqs) before continuing.

-  Log in to the development cluster's CLI (Kubernetes or Red Hat OpenShift)

-  To do so, navigate to your cluster in the _IBM Cloud console_, click on the **Access** tab, and follow the instructions in the **After your cluster provisions, gain access** section to log in to the cluster from the command line.

**NOTE**: In case, your workshop is on Code Ready Workspaces,follow the steps in [Code Ready Workspaces Setup](/getting-started/crwsetup) before logging in to the cluster. 
...
- Run the following command to get the Developer Dashboard link :
    ```
    igc dashboard
    ```
...
Most developers like to run the code once. To do so, follow the instructions listed in the **README.md** file to run the code locally. You may be required to install a specific runtime like **Java**, **Node**, or **Python**.


### Register the App in a DevOps Pipeline

You now have the code in a GitHub repository and have cloned it to your workspace in CRW. You now need to register the repository with the continuous integration pipeline. The <Globals name="env" /> supports both [Tekton](/guides/continuous-integration-tekton) and [Jenkins](/guides/continuous-integration) for continuous integration.
...
Create your project namespace first using the below command:

    igc namespace dev-{initials} --jenkins --tekton
    
**NOTE**: Check if the project namespace is created by listing the projects.
...
<AccordionItem title="OpenShift">
  ...
  If the project is not created,run
  ...
  For example:
  ```bash
  oc new-project dev-rr
  igc namespace dev-rr --jenkins --tekton
   ```
  ...
  For example:
  ```bash
  kubectl create namespace dev-rr
  igc namespace dev-rr --jenkins --tekton
   ```
...
<Tab label="Tekton">
  ...
  CLI will additionally ask for the **Pipeline** to be used in the PipelineRun.
  
  The output looks like this:

    $igc pipeline -n dev-rr --tekton
    Creating pipeline on openshift cluster in dev-rr namespace
    Getting git parameters
    ? Provide the username: ramyaraghuveera
    Project git repo: https://github.com/RamyaRaghuveera/inv-svc-test.git
    ...
    ? Select the Pipeline to use in the PipelineRun: 
    igc-appmod-liberty-v1-2-0 
    igc-java-gradle-v1-2-0 
    igc-java-maven-v1-2-0 
    ❯ igc-nodejs-v1-2-0 
    Skip PipelineRun creation 
    Select the Pipeline to use in the PipelineRun: igc-nodejs-v1-2-0
    Copying tasks from tools
    Copied Pipeline from tools/igc-nodejs-v1-2-0 to dev-rr/inv-svc-test
    Creating PipelineRun for pipeline: inv-svc-test
...
<Tab label="Jenkins">
  ...
  The output looks like this:
    ...
    Registering pipeline: RamyaRaghuveera.inv-svc-test
    Creating git webhook
    Creating webhook to url:  https://c100-e.us-east.containers.cloud.ibm.com:31919/apis/build.openshift.io/v1/namespaces/dev-rr/buildconfigs/ramyaraghuveera.inv-svc-test/webhooks/secret101/github
...
![OpenShift](images/ocp43Pipeline.png)

</Tab>

<Tab label="Kubernetes">

- Run the command `igc dashboard`  to get  your [<Globals name="dashboard" />](/getting-started/dashboard) url and open it in a browser.
...
- The build pipeline is configured to build you source code into a container image. This image is stored in the [Image Registry](/practical/image-registry). The image is then deployed into your namespace/project within you development cluster and validated for its health.
...
- On the left side menu, go to **Workloads**->**Deployments**
...
![Pods on OpenShift](images/ocp43Application.png)
