...
activation content and Starter Kits to initiate a project quickly.
...
- [`credentials`](#credentials): shows information about the same tools shown in the dashboard from the command-line.
- `[tool-config`](#tool-config): allows configuration for additional tools to be added to the cluster, making them available to the dashboard and `credentials` command.
